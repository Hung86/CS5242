...
class LSTMCell(Layer):
    "Only for testing onestep lstm"

    def __init__(self, in_features, units, name='lstm_cell', initializer=Gaussian()):
        """
        # Arguments
            in_features: int, the number of input features
            units: int, the number of hidden units
        ...
        self.kernel = initializer.initialize((in_features, 4 * units))
        self.recurrent_kernel = initializer.initialize((units, 4 * units))
    ...
    def forward(self, input):
        ...
        # Returns
            outputs: [hidden state numpy array with shape (batch, units),
                    cell state numpy array with shape (batch, units)]
        """
